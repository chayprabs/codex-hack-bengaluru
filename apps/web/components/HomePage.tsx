...
export function HomePage() {
  ...
  async function handleAuditClick() {
    ...
    setFeedback({
      tone: "neutral",
      message: `Submitting ${trimmedUrl} to the audit API and preparing your audit room.`,
    });
        ...
        <div className="grid flex-1 gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div className="max-w-3xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-slate-200/90 bg-white/90 px-4 py-2 text-sm text-slate-700 shadow-sm backdrop-blur">
              <span className="font-semibold text-slate-900">Live audit workspace</span>
              Typed API calls, clear findings, and resilient room updates
            </div>
                ...
                {[
                  ...
                  <div key={title} className="rounded-2xl border border-slate-200 bg-slate-50/85 p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">
                        queued
                      </span>
                    </div>
